...
function noBookedCallEmail(input: LeadCaptureInput | FollowupInput, reason: 'lead_logged' | 'followup_booked') {
  ...
  const text = [
    ...
    '',
    `If texting is easier, text or call us at ${SENDBLUE_DISPLAY_NUMBER}. A quick text like "call me" or "send details" is enough and we will help from there.`,
    '',
...
function bookedCallEmail(input: LeadCaptureInput | FollowupInput, appointment: AppointmentResult) {
  ...
  const text = [
    ...
    '',
    `If anything changes before the call, text or call us at ${SENDBLUE_DISPLAY_NUMBER}. A quick text is fine. We are always here to help.`,
    '',
    'Talk soon,',
    'Saul',
    'AskSaul.ai',
