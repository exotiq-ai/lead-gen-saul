...
async function fetchScoringData(tenantId: string): Promise<ScoringData> {
  ...
  const score_by_stage = Array.from(stageMap.entries())
    .map(([stage, scores]) => ({ stage, count: scores.length, avg_score: round1(avg(scores)) }))
  ...
  const score_by_source = Array.from(sourceMap.entries())
    .map(([source, scores]) => ({ source, count: scores.length, avg_score: round1(avg(scores)) }))
    .sort((a, b) => b.avg_score - a.avg_score)
  ...
  for (const l of rows) {
    ...
    for (const flag of flags) {
      if (flag?.code) flagCounts.set(flag.code, (flagCounts.get(flag.code) ?? 0) + 1)
    }
  }
  const red_flag_breakdown = Array.from(flagCounts.entries())
    .map(([code, count]) => ({ code, count, severity: FLAG_SEVERITY[code] ?? 'medium' }))
